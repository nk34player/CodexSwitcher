...
1. **Dashboard Page**
   - A large glowing action area for launching Codex or switching into the selected profile.
   - Plan badges showing the detected account tier for the active profile.
   - Taller, roomier glass panels sized for a high-aspect-ratio desktop layout so controls and analytics widgets have more breathing room.
   - Statistics cards for daily usage, weekly activity, conversation threads, and autonomous agent jobs.
   - Inline explanations under each metric describing what local files are being summarized and why the data stays on-device.
2. **Manage Profiles**
   - Create profiles with custom emoji avatars and color treatments.
   - Reuse trusted local Codex configuration as a starting point for new profiles.
3. **System Backups**
   - Create, restore, and delete timestamped backups from the app.
   - Review backup size information directly in the backups manager.
4. **App Settings**
   - Override the Codex executable path when needed.
   - Enable or disable the app lock with a secure passcode stored in the keychain.
   - Trigger a panic reset that clears CodexSwitcher data without modifying Codex itself.

### Dashboard Page Details

- Daily and weekly widgets no longer show arbitrary fractional counters such as `0/15`.
- When live remaining-capacity data is available from locally cached Codex account metadata, the dashboard shows that directly.
- When live limit data is unavailable, the widgets fall back to local historical counts gathered from on-device Codex files.
- Each metric includes an inline explanation describing what is being counted and why the computation stays local for privacy.
